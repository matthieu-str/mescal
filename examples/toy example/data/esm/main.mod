...
param layers_in_out {RESOURCES union TECHNOLOGIES, LAYERS} default 0;
...
subject to end_uses_t {l in LAYERS, t in PERIODS}:
	End_Uses [l,t] = (if l == "ELECTRICITY" then
			(end_uses_input[l] / total_time + end_uses_input["LIGHTING"] * lighting_month [t] / t_op [t] + end_uses_input["HEAT_LOW_T_SC"] * cooling_month [t] / t_op [t])
		else 
...
subject to layer_balance {l in LAYERS, t in PERIODS}:
	0 = (sum {i in RESOURCES union TECHNOLOGIES} (layers_in_out[i, l] * F_Mult_t [i, t])
		- End_Uses [l, t]
		);
...
subject to investment_cost_calc_1 {i in TECHNOLOGIES}:
	C_inv [i] = c_inv [i] * F_Mult [i];

# [Eq. 1.4] O&M cost of each technology
subject to main_cost_calc {i in TECHNOLOGIES}:
...
subject to production2{i in TECHNOLOGIES}:
