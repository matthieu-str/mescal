...
to reproduce when another ESM is used. This complicates the re-application and comparability of these approaches. 
...
Pre-computed LCA impact scores are used to compute to total environmental impact of the modelled energy system 
configuration.
...
Before the LCI and LCIA steps, `mescal` modifies the LCI datasets intermediary and elementary flows to regionalize, 
...
`mescal` operates this set of systematic transformations and computations based on the `brightway` 
...
`mescal` can control the difference in order of magnitude between the highest and lowest impact scores of an impact 
...
Updating the LCI database background inventory with ESM results paves the way for using `mescal` with both snapshot and 
myopic pathway ESM.
Snapshot ESM evaluate the energy system configuration and operation over a timespan, during which the energy system 
remains unchanged [@codinagirones2015]. An iterative feedback loop can be established between the ESM and `mescal` 
