...
LCA is a standardized modelling approach that assesses the environmental impacts of products and services throughout their life-cycle. 
LCA aims to be a comprehensive environmental assessment as it considers complete value chains and a wide set of 
environmental impact categories.
LCA combines unit process descriptions (e.g., the purchases and emissions of a coal extraction process, or coal 
transport, etc.) to calculate a Life-Cycle Inventory (LCI): a quantification of all environmental stressors linked 
directly or indirectly to the fulfillment of a function (e.g., all emissions of CO$_2$, CH$_4$, SO$_2$, etc. linked directly or 
indirectly with the delivery of 1 kWh of electricity). This LCI is then used to calculate the Life-cycle Impact 
Assessment (LCIA) of the function (e.g., all global warming and acid rain impacts linked to 1 kWh of electricity).
...
Model (IAM), prospective LCI databases (which are typically generated using `premise` [@sacchi2022], 
...
framework [@mutel2017] and the `wurst` Python package, which are open-source tools to perform LCA modelling and calculation 
within a Python environment.

![`mescal`'s iterative workflow for a hard-linked ESM-LCA modelling.\label{fig:workflow}](docs/pics/workflow_v2.png)

`mescal` requires four mandatory CSV files for data input:
...
inventory can be regionalized using the open-source `regioinvent` library [@maximeagez2025].
...
Double-counting refers to the multiple occurrence of the same energy flow within the energy system model, thus leading 
